...
"""
High‑level flow
1️⃣  Customer says “hi / hello / menu”. Bot checks Shopify for the phone:
    • Existing → greeting + *Menu* button.
    • New      → registration + *Sign‑Up* CTA.

2️⃣  Main menu (interactive):
    • Catalog → Shopify collections → products → variants.
    • Custom Jewellery → ask for the piece name → thank‑you & finish.

3️⃣  Variant selection → add to a temporary cart → ask **Retail** or **B2B**.

4️⃣  Customer types **checkout** → bot totals the cart,
    creates a Razorpay payment‑link, and sends a “Pay Now” CTA.
    Payment‑link message is generated by Gemini (different for Retail vs B2B).

5️⃣  Razorpay callback (GET) or webhook (POST) notifies the user:
    • B2B → “Download Now” button (digital 3‑D file).
    • Retail → “We will contact you soon” message.

6️⃣  Any other Shopify‑related query (order status, gift‑card balance,
...
7️⃣  Cron job (21‑23 h) → reminder “🛒 aapka cart abhi bhi hai – checkout ya menu dabayein”.
...
# ------------------------------------------------------------------
# 2️⃣  Environment variables (add them in Render → Environment)
# ------------------------------------------------------------------
...
VERIFY_TOKEN        = env("VERIFY_TOKEN")                     # Meta webhook verify token
ACCESS_TOKEN        = env("ACCESS_TOKEN")                     # Meta page token (send messages)
...
SHOPIFY_CLIENT_ID   = env("SHOPIFY_CLIENT_ID")                # Private‑app client ID
SHOPIFY_CLIENT_SECRET = env("SHOPIFY_CLIENT_SECRET")          # Private‑app client secret
RAZORPAY_KEY_ID     = env("RAZORPAY_KEY_ID")                  # Razorpay test/live key ID
RAZORPAY_KEY_SECRET = env("RAZORPAY_KEY_SECRET")              # Razorpay test/live secret
GEMINI_API_KEY      = env("GEMINI_API_KEY")                   # Gemini 1.5 Flash API key
GOOGLE_CRED_JSON    = env("GOOGLE_CREDENTIALS", required=False)  # Service‑account JSON (optional)
SHEET_ID            = env("SHEET_ID", required=False)         # Google Sheet ID (optional)
...
# ------------------------------------------------------------------
# 4️⃣  OAuth token cache (thread‑safe)
# ------------------------------------------------------------------
_token_cache = {
    "access_token": None,
    "expires_at": 0,
...
def _request_new_shopify_token() -> dict:
    """OAuth client‑credentials request – returns token payload."""
...
def get_shopify_access_token() -> str:
    """Return a valid X‑Shopify‑Access‑Token, refreshing when needed."""
    with _token_cache["lock"]:
        ...
        log.info(
            "Fetched new Shopify token (expires in %s secs)",
            data.get("expires_in"),
        )
        return _token_cache["access_token"]
...
# ------------------------------------------------------------------
# 5️⃣  Session data class
# ------------------------------------------------------------------
@dataclass
class UserSession:
    phone: str
    step: str = "new"                     # new, old_greeted, awaiting_…, browsing, payment_pending, …
...
# In‑memory store (lost on container restart – fine for demo)
sessions: Dict[str, UserSession] = {}

# ------------------------------------------------------------------
# 6️⃣  Gemini system prompts (Hinglish – Latin script)
# ------------------------------------------------------------------
SYSTEM_PROMPTS: Dict[str, str] = {
    ...
    "custom_jewellery": (
        "Tu A Jewel Studio ka WhatsApp assistant hai. "
        "Customer custom jewellery banana chahta hai. Poora naam maango aur thank‑you ke saath end karo. "
        "Tone: Professional, friendly, Hinglish. 2‑3 lines max."
    ),
    "customer_type": (
        "Tu A Jewel Studio ka WhatsApp assistant hai. "
    ...
    "retail_confirm": (
        "Retail customer ka order receive hua. Professional thank‑you do. "
    ...
    "b2b_success": (
        "B2B customer ki payment successful rahi. Warm thank‑you do aur batao files share ho gayi hain. "
        "Tone: Warm, professional, Hinglish. 3‑4 lines max."
    ),
    "b2b_failed": (
...
def gemini_reply(user_msg: str, ctx: str = "general", cust_name: str = "") -> str:
    """Call Gemini‑1.5‑Flash and return plain text; on failure return tiny fallback."""
...
# ------------------------------------------------------------------
# 8️⃣  Helper – generic Shopify GET (uses OAuth token)
# ------------------------------------------------------------------
def shopify_get(endpoint: str) -> dict:
    """GET against Shopify Admin API using the cached access token."""
...
# ------------------------------------------------------------------
# 9️⃣  Helper – optional Google Sheet logging
# ------------------------------------------------------------------
...
def create_razorpay_link(amount_inr: float, order_id: str, name: str, phone: str) -> str:
    """Create a Razorpay payment link; return its short URL (or empty string)."""
    payload = {
        "amount": int(amount_inr * 100),  # paisa
        "currency": "INR",
        "description": f"A Jewel Studio Order {order_id}",
        "customer": {"name": name, "contact": phone},
        "callback_url": "https://<your-app>.onrender.com/payment-callback",  # ← replace with your domain
        "callback_method": "get",
...
# ------------------------------------------------------------------
# 11️⃣  UI helpers – catalog & cart flow
# ------------------------------------------------------------------
...
def add_variant_to_cart(sess: UserSession, variant_id: str):
    """Add variant → ask Retail vs B2B immediately."""
    sess.cart.append(int(variant_id))
    wa_send(
        sess.phone,
        "✅ Added to cart! अब ग्राहक प्रकार चुनें (Retail / B2B).",
        "interactive",
        {
            "type": "button",
...
# ------------------------------------------------------------------
# 12️⃣  Checkout & payment flow
# ------------------------------------------------------------------
def start_checkout(sess: UserSession) -> None:
    """Calculate total, create Razorpay link, send Pay‑Now CTA."""
    total = 0.0
...
def payment_success(sess: UserSession) -> None:
    ...
    if sess.customer_type == "b2b":
        msg = gemini_reply("Payment successful – B2B", "b2b_success", "")
        ...
        msg = gemini_reply("Payment successful – Retail", "retail_confirm", "")
        wa_send(sess.phone, msg)
        wa_send(
            sess.phone,
...
def payment_failed(sess: UserSession) -> None:
    """Ask user to retry after a failed payment."""
...
# ------------------------------------------------------------------
# 13️⃣  Intent detector (free‑hand Shopify queries)
# ------------------------------------------------------------------
def detect_shopify_intent(text: str):
    """Very light keyword‑based intent matcher."""
    t = text.lower()
...
# ------------------------------------------------------------------
# 14️⃣  Shopify‑related intent handlers
# ------------------------------------------------------------------
def fetch_customer_by_phone(phone: str) -> Optional[dict]:
    """Return first Shopify customer matching the phone."""
...
def handler_order_status(phone: str, params: dict) -> str:
    ...
    if not cust:
        return "🙁 Aapke phone number se koi customer record नहीं मिला."
    ...
    if not orders:
        return "❌ Aapke liye abhi tak koi order नहीं है."
...
def handler_gift_card(phone: str, _: dict) -> str:
    cust = fetch_customer_by_phone(phone)
    if not cust:
        return "🙁 Aapke phone number se koi customer नहीं मिला."
    gc_resp = shopify_get(f"customers/{cust['id']}/gift_cards.json")
    cards = gc_resp.get("gift_cards", [])
    if not cards:
        return "💳 Aapke paas अभी कोई active gift‑card नहीं है."
    lines = ["🪙 Aapke active gift‑cards:"]
    for c in cards:
        bal = int(c.get("balance", 0)) / 100
        code = c.get("code", "N/A")
        expiry = c.get("expires_on", "N/A")
        lines.append(f"• Code: {code} | Balance: ₹{bal:.2f} | Expiry: {expiry}")
    return "\n".join(lines)
...
def handler_discounts(_: str, __: dict) -> str:
    ...
    if not rules:
        return "📉 Filhal koi active discount code नहीं है."
    lines = ["🎁 Hamare latest discount codes:"]
    for r in rules:
...
def handler_inventory(_: str, params: dict) -> str:
    ...
    if not product_name:
        return "⚠️ Stock check करने के लिये product name बताइए."
    resp = shopify_get(f"products.json?title={product_name}")
    products = resp.get("products", [])
    if not products:
        return f"❌ {product_name} hamare catalog में नहीं मिला."
    ...
    return f"✅ {product_name} ka current stock: {qty} units."


def handler_product(_: str, params: dict) -> str:
    product_name = params.get("product", "").strip()
    if not product_name:
        return "⚠️ Product name लिखिए, फिर मैं details भेजूँगा."
    resp = shopify_get(f"products.json?title={product_name}")
    products = resp.get("products", [])
    if not products:
        return f"❌ {product_name} हमारे catalog में नहीं है."
...
def handler_returns(_: str, __: dict) -> str:
    ...
    if not returns:
        return "📦 Aapke liye अभी तक कोई return नहीं है."
    lines = ["🔙 Recent returns:"]
    for r in returns[:5]:
        oid = r.get("order_id", "N/A")
        status = r.get("status", "N/A")
        lines.append(f"• Order {oid}: {status}")
    return "\n".join(lines)
...
def handler_customer(_: str, params: dict) -> str:
    ...
    cust = fetch_customer_by_phone(phone)
    if not cust:
        return "🙁 Aapka customer profile नहीं मिला."
...
# ------------------------------------------------------------------
# 15️⃣  Flask routes – webhook verification & inbound messages
# ------------------------------------------------------------------
...
def inbound():
    ...
    try:
        ...
        # ------------------------------------------------------------
        # 1️⃣ New / Existing customer detection (runs once)
        # ------------------------------------------------------------
        if sess.step == "new":
        ...
        # ------------------------------------------------------------
        # 2️⃣ Interactive button replies (catalog flow, custom jewellery)
        # ------------------------------------------------------------
        if msg_type == "interactive":
            ...
            if sess.step == "await_name_custom":
                sess.name = btn_id          # WhatsApp sends the entered text as the button ID
                wa_send(
                    phone,
        ...
        # ------------------------------------------------------------
        # 3️⃣ Text messages (commands, free‑hand queries, generic chat)
        # ------------------------------------------------------------
        if msg_type == "text":
            text = msg["text"]["body"].strip()

            # ---- 1️⃣ Check for a Shopify‑related free‑hand intent ----
            ...
            if low == "checkout":
                if not sess.cart:
                    ...
                    wa_send(phone, "⚙️ Pehle customer type (Retail / B2B) choose karein.")
                else:
                    start_checkout(sess)
                return jsonify({"status": "ok"}), 200

            # ---- 3️⃣ Data‑capture steps (name, contact, etc.) ----
            ...
            # ---- 4️⃣ Generic fallback – let Gemini handle any other chat ----
...
# ------------------------------------------------------------------
# 16️⃣  Razorpay GET callback (after user pays)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 17️⃣  Razorpay POST webhook (more reliable alternative)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 18️⃣  Daily reminder – Render cron (21‑23 h)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 19️⃣  Run Flask (Render injects $PORT)
# ------------------------------------------------------------------
