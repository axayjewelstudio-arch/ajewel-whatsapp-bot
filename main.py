...
def send_new_customer_welcome(to_number):
    ...
    if LOGO_IMAGE_URL:
        send_whatsapp_image(to_number, LOGO_IMAGE_URL, caption="Welcome to\n\n*A Jewel Studio*")
        time.sleep(2)
    
    join_url = f"{JOIN_US_URL}?wa={to_number}"
    send_whatsapp_cta_button(to_number, "Tap Join Us below to become a part of our family and explore our exclusive collections.", "Join Us", join_url)
...
def send_complete_registration(to_number):
    ...
    join_url = f"{JOIN_US_URL}?wa={to_number}"
    send_whatsapp_cta_button(to_number, message, "Complete Registration", join_url)
